...
public class RootMotionControlScript : MonoBehaviour
{
    ...
    // Simple features that will allow character to move and turn futher while mostly preserving roote motion
    public float animationSpeed = 1f;
    public float rootMovementSpeed = 1f;
    public float rootTurnSpeed = 1f;

    // For IK (button pressing)
    ...
    void OnAnimatorMove()
    {
        ...
        this.transform.position = Vector3.LerpUnclamped(transform.position, newRootPosition, rootMovementSpeed);
        this.transform.rotation = Quaternion.LerpUnclamped(transform.rotation, newRootRotation, rootTurnSpeed);
    	...
    	if (anim) {
			...
			if(astate.IsName("ButtonPress")) {
				...
				if(buttonObject != null) {
					anim.SetLookAtWeight(buttonWeight); anim.SetLookAtPosition(buttonObject.transform.position); anim.SetIKPositionWeight(AvatarIKGoal.RightHand,buttonWeight); anim.SetIKPosition(AvatarIKGoal.RightHand,
